...
\author{Joaquin Matres,\authormark{1,*} Simon Bilodeau,\authormark{3} Niko Savola,\authormark{2,4} Marc de Cea,\authormark{2} Wai Kwan Yeung,\authormark{5} Erman Timurdogan,\authormark{7} Helge Gehring,\authormark{6} and Troy Tamas\authormark{1}}
...
GDSFactory implements cells as Python functions returning a \texttt{Component} class with polygons, port metadata, and convenience methods. Using KLayout's C++ geometry engine~\cite{klayout}, users define PCells with a functional programming approach:
...
GDSFactory's gplugins repository provides unified interfaces to device simulators by reusing layout abstractions. Components can be meshed via GMSH for cross-sectional or 3D analysis (Fig.~\ref{fig:mesh}).
...
Full-wave electromagnetic simulation is supported through three FDTD backends: MEEP~\cite{meep} (open-source), Tidy3D~\cite{tidy3d} (cloud-based GPU acceleration), and Lumerical FDTD. FEM and multiphysics solvers include Femwell~\cite{femwell} for waveguide mode analysis and thermal simulation, Palace~\cite{palace} for RF and microwave simulations, MPB for photonic crystals, and MEOW for eigenmode expansion. TCAD simulation uses DEVSIM~\cite{devsim} for semiconductor device physics and Sentaurus for advanced process simulation.
