...
GDSFactory's gplugins repository~\cite{gplugins} provides unified interfaces to device simulators by reusing layout abstractions. Components can be meshed via GMSH \cite{gmsh} (through a wrapper \cite{meshwell}) for cross-sectional or 3D analysis (Fig.~\ref{fig:mesh}).
...
GDSFactory provides a unified Python-driven workflow spanning layout design, device simulation, and circuit simulation. The tight integration between device solvers (Palace, Femwell, DEVSIM), and circuit simulators (SAX) enables rapid design iteration from component to system level. The library is freely available at \url{https://github.com/GDSFactory/GDSFactory}.
