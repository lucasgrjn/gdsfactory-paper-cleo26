...
\author{Joaquin Matres,\authormark{1,*} Simon Bilodeau,\authormark{2} Niko Savola,\authormark{3,4} Marc de Cea,\authormark{3} Wai Kwan Yeung,\authormark{5} Erman Timurdogan,\authormark{6} Jan David Fischbach,\authormark{7} Helge Gehring,\authormark{8} Floris Laporte,\authormark{1} Sebastian Goeldi,\authormark{1} and Troy Tamas\authormark{1}}
...
\authormark{3}Taara Connect, Inc., Sunnyvale, CA 94089, USA\\
...
\authormark{6}Lumentum, San Jose, CA, USA\\
\authormark{7}Institute of Nanotechnology, Karlsruhe Institute of Technology (KIT), Karlsruhe, Germany\\
\authormark{8}Institute of Physics and Center for Nanotechnology, University of Münster, Münster D-48149, Germany}
...
\begin{abstract}
We present GDSFactory, an open-source Python library for integrated circuit design automation supporting photonics, analog, quantum, and MEMS applications. The platform provides unified workflows for full chip development: layout design and simulations (device and circuit via S-parameter analysis).
\end{abstract}

\section{Introduction}

Hardware iterations typically require months and millions of dollars, whereas software iterations cost hundreds of dollars and take hours. GDSFactory bridges this gap by providing a comprehensive and complete Python API for chip development, including layout design, device simulation, circuit simulation, and verification~\cite{gdsfactory}.

In contrast to traditional logic-driven electronic design flows~\cite{bogaerts2018}, integrated photonics requires freeform parametric geometries and tight integration between layout and electromagnetic simulation. GDSFactory addresses this with scriptable parametric cells (PCells), hierarchical assembly, routing, and seamless interfaces to industry-standard simulators while leveraging Python's extensive scientific ecosystem~\cite{awesome} (Fig.~\ref{fig:flow}).
...
GDSFactory's gplugins repository~\cite{gplugins} provides unified interfaces to a growing list of simulators by reusing the core layout abstractions (Components, Layerstacks, Ports). For instance, Components can be meshed via GMSH \cite{gmsh} (through a wrapper \cite{meshwell}) for cross-sectional or 3D analysis (Fig.~\ref{fig:mesh}) For rigorous electromagnetic verification required by photonics, the library supports full-wave analysis via both Finite-Difference Time Domain (FDTD) and Finite Element Method (FEM) approaches. The FDTD method is supported through open-source backends including MEEP~\cite{meep} and Luminescent AI, as well as solvers commonly used in industry such as Tidy3D~\cite{tidy3d} (cloud-based GPU acceleration) and Lumerical FDTD. Available FEM-based and multiphysics solvers include Femwell~\cite{femwell} for waveguide mode analysis and thermal simulation, Palace~\cite{palace} for RF and microwave simulations, and MEOW \cite{meow} for eigenmode expansion. For lower-level modeling, TCAD simulations are also supported through DEVSIM~\cite{devsim} for semiconductor device physics and Sentaurus for advanced process simulation. % chktex 13
...
Circuit-level simulation enables system-scale photonic design. GDSFactory facilitates this through netlist extraction, which enables compositions of device-level simulations. For example, we have used SAX~\cite{sax} for differentiable S-parameter circuit simulation using JAX, which supports automatic differentiation for gradient-based optimization, Monte Carlo analysis for yield estimation, and wavelength-dependent S-parameter interpolation from FDTD results. VLSIR provides SPICE netlist export for mixed photonic-electronic simulation.
